...
function onSpotifyPlayerAPIReady() {
    ...
    SpotifyPlayer.player = new Spotify.Player({
    ...
    SpotifyPlayer.player.on('ready', function (e) {
      ...
      notice.innerHTML = '<b>' + SpotifyPlayer.player._options.name + '</b> successfully connected with Device ID <b>' + e.device_id + '</b>';
    ...
    SpotifyPlayer.player.on('player_state_changed', function (e) {
      ...
      let currentTrack = e.track_window.current_track;
      track_name.innerHTML = currentTrack.name;
      track_name.href = 'http://open.spotify.com/track/' + currentTrack.id;
      artist_name.innerHTML = currentTrack.artists[0].name;
      artist_name.href = 'http://open.spotify.com/artist/' + currentTrack.artists[0].uri.split(':')[2];
      album_art.src = currentTrack.album.images[0].url;
    });
  
    // Handle errors
    SpotifyPlayer.player.on('initialization_failed', function (e) {
      console.log('Initialization Failed', e);
      document.getElementById('notice').innerHTML = '<b>Initialization Failed.</b> ' + e.toString();
    });
    SpotifyPlayer.player.on('authentication_error', function (e) {
      console.log('Authentication Error', e);
      document.getElementById('notice').innerHTML = '<b>Authentication Error.</b> ' + e.toString();
    });
    SpotifyPlayer.player.on('account_error', function (e) {
      console.log('Account Error', e);
      document.getElementById('notice').innerHTML = '<b>Account Error.</b> ' + e.toString();
    });
    SpotifyPlayer.player.on('playback_error', function (e) {
      console.log('Playback Error', e);
      document.getElementById('notice').innerHTML = '<b>Playback Error.</b> ' + e.toString();
    });
